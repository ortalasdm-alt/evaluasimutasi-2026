...
async function loadPegawaiFromGAS() {
  try {
    ...
    data.forEach(row => DAFTAR_PEGAWAI.push({
      nama: row.nama || row.nama_pegawai || '',
      nip: row.nip || row.nip_nik || '',
      jabatan: row.jabatan || row.jabatan_pegawai || '',
      satuan_kerja: row.unit || row.unit_kerja || ''
    }));
